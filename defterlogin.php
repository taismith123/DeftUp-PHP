...
// direct defter to welcome page if he/she is already logged in
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
    header("location: defterWelcome.php");
    exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($userError) && empty($passError)){
        //connect to database
        $sql = "SELECT id, username, password FROM defters WHERE username = ?";
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashPassword)){
                            ...
                            // redirect defter to the welcome page
                            header("location: defterWelcome.php");
                        } else{
...
<div class="wrapper">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        <div class="form-group">
            ...
            <input type="text" name="username" class="form-control <?php echo (!empty($userError)) ? 'is-invalid' : ''; ?>" value="<?php echo $username; ?>">
            <span class="errorMessages"><?php echo $userError; ?></span>
        </div>
        <div class="form-group">
            <label>Password</label>
            <input type="password" name="password" class="form-control <?php echo (!empty($passError)) ? 'is-invalid' : ''; ?>">
