...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT id FROM defters WHERE username = ?";
    ...
    if(empty($usernameError) && empty($passwordError) && empty($confirmPassError)){
        ...
        $sql = "INSERT INTO defters (username, password) VALUES (?, ?)";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                // redirect defter to designated login page
                header("location: defterlogin.php");
            } else{
...
<div class="wrapper">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        ...
        <div class="form-group">
            ...
            <input type="password" name="confirm_password" class="form-control <?php echo (!empty($confirmPassError)) ? 'is-invalid' : ''; ?>" value="<?php echo $confirm_password; ?>">
        ...
        <p>Already have an account? <a href="defterlogin.php">Login here</a>.</p>
    </form>
</div>
</body>
</html>